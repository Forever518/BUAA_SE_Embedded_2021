<template>
  <div class="bodyBoxOut">
    <el-row type="flex" justify="start" v-if="this.maps.length !== 0">
      <el-col :span="6" v-for="map in maps" :key="map.url" :offset="1" style="width: 300px">
        <el-card :body-style="{padding: '5px'}" shadow="hover" style="width: 300px">
          <!-- style="width: 260px;height: 320px;" -->
          <div style="padding: 6px;">
            <div style="margin: 0 auto; overflow: hidden; zoom: 1 ;width: 154.8px">
    ...
    methods: {
      getMap() {
        post_getMap().then((res) => {
          ...
          this.maps = mapList;
          this.mapsPre = JSON.parse(JSON.stringify(mapList));
        })
      },
      deleteMap(map) {
        let _this = this;
      ...
      editFin(map) {
        ...
        console.log(this.mapsPre);
        map.nameMode = 'show';
        post_changeMapName({
          mapPreviousName: this.mapsPre[this.maps.indexOf(map)].name,
          mapNewName: map.name,
        }).then((res) => {
          if (res.code !== 200) {
            map.name = this.mapsPre[this.maps.indexOf(map)].name;
          }
        })
      }
    },
