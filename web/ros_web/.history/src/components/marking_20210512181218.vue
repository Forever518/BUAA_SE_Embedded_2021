...
<script>
import {post_getWayPoint, post_renameMark, post_saveMark} from '../api/request';
  export default {
    data() {
      return {
        dialogVisible: false,
    ...
    methods: {
      ...
      wayPointName() {
        ...
        if (!wrong) {
          ...
          console.log(this.dynamicForm.wayPoints)
          for (let wayPoint in this.dynamicForm.wayPoints) {
            tempIndexes['' + wayPoint.label + ''] = wayPoint.name;
            console.log(wayPoint.label, wayPoint.name);
          }
