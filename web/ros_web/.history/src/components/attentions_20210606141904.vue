      ...
      <el-dialog
        ...
        <el-form :model="form" @submit.native.prevent>
          <el-form-item label="地图">
            <el-select v-model="form.mapName" filterable clearable placeholder="请选择地图" style="width: 80%!important">
              <el-option
                ...
                :label="map.name"
                :value="map.name">
                <span style="float:left; height:100%; line-height: 160px"> {{map.name}} </span>
...
<script>
import {post_createMap, post_startMark, post_startService, post_getMap, post_getDirect} from '../api/request';
  export default {
   data() {
     return {
       haveRead: false,
   ...
   methods: {
     ...
     get_attention_data() {
       let dirName = '';
       if (this.type === 'build') {
         dirName = 'building_attentions';
       } else if (this.type === 'mark') {
         dirName = 'marking_attentions';
       } else {
         dirName = 'serving_attentions';
       }
       post_getDirect({
         name: dirName
       }).then((res) => {
